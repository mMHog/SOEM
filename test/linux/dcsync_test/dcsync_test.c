   ...
   if (ec_init(ifname))
   {
      ...
      if (ec_config_init(FALSE) > 0) // == ec_config_init + ec_config_map
      {
         ...
         if (ec_slave[0].state == EC_STATE_OPERATIONAL)
         {
            ...
            {
               printf("%d to zero\n", k);
               num = data_process(outputx, rad2inc(0, k), init_position[k], 200, 10, 0.1);
               for (int i = 0; i < num; ++i)
               {
            ...
            for (int k = 0; k < SERVO_NUMBER; ++k)
            {
               ...
               printf("%d to init\n", k);
               num = data_process(outputx, rad2inc(load_data[k],k), rad2inc(0,k), 200, 10, 0.1);
               for (int i = 0; i < num; ++i)
               {
                  commend[k]->target_position = (int)outputx[i];
                  osal_usleep(50);
